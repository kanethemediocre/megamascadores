class Mquest{
	constructor(answer,quantity,origin,originmap,type,reward){
		this.answer = answer;
		this.answermax = answer;
		this.quantity = quantity;
		this.gotten = 0;
		this.originnpc = origin;
		this.originmap = originmap;
		this.type = type;//"=" for only exact matches to this.answer, "*" is all multiples of this.answer
		this.reward = reward;//"somekindofupgradegoeshere";//something goes here
		this.taken = false;
		this.active = false;
		this.visible = true;
	...
	check(theanswer){
		...
		else if ( (this.type=="range")&&(theanswer>=this.answer)&&(theanswer<=this.answermax) ){pass=true;}
		}
	turnin(){
