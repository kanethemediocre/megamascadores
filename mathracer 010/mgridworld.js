class Mgridworld{
	...
	addeqcolumn(hf,numeqs,x){
	//var randomy = Math.floor(testgridworld.n*Math.random());
		var num = numeqs;
		if (num>this.n){num = this.n;}//does not allow for more elements than size of column
		var ys = [];
		var i=0;
		while(i<this.n){//Fills array with all the indexes
			ys.push(i);
			i++;
			}
		while(ys.length>numeqs){//removes random elements if num is less than size of column
			ys.splice(Math.floor(Math.random()*ys.length), 1);
			}
		var i=0;
		while(i<ys.length){//places randomized equations
			testgridworld.grid[x][ys[i]].sprout();
			testgridworld.grid[x][ys[i]].eq.growth = [1,1,1];
			testgridworld.grid[x][ys[i]].eq.finish();
			i++;
			}
		var thegoodone = Math.floor(Math.random()*ys.length);
		while(testgridworld.grid[x][ys[thegoodone]].eq.s!=hf){//Just keeps trying until it gets the right answer.  Infinite loop danger zone
			testgridworld.grid[x][ys[thegoodone]].sprout();
			testgridworld.grid[x][ys[thegoodone]].eq.growth = [1,1,1];
			testgridworld.grid[x][ys[thegoodone]].eq.finish();
			}
		}
	...
	draw(viewx,viewy,xstep,ystep){
		...
		var myxrange = Math.floor((canvas.width/2)/xstep) +10;//Could do at initialization and save as parameter
		...
		while(i<this.players.length){
			...
			var py = this.players[i].yyy-viewy;//ystep*(this.players[i].y+0.05)-viewy;
			
			var px = this.players[i].xxx-viewx;
	...
	update(){
		...
		while(i<this.players.length){//Handle player movement
			...
			var mytile = this.grid[this.players[i].x][this.players[i].y];
			if ((mytile.eq.s==this.players[i].targetanswer)&&(mytile.c=="white")){
				this.players[i].vx = this.players[i].vx+4;
				var oldtarget = this.players[i].targetanswer;
				while (oldtarget == this.players[i].targetanswer){//keep randomizing until a new answer is reached.
					this.players[i].targetanswer = Math.floor(Math.random()*9)+1;
					}
				mytile.c="green"
				}
			else if ((mytile.c=="white")&&(mytile.eq.complete)){//could restructure to check if white only once.
				this.players[i].vx = 0;
				mytile.c="red";
				}
			//else if (mytile.eq.s>0){//check if active tile
			//	this.players[i].vx = this.players[i].vx-2;
			//	}
			if (mytile.blocking){
				console.log("itriedtoblock");
				this.players[i].vx = this.players[i].vx - 2;
				}
			
			this.players[i].xxx = this.players[i].xxx + this.players[i].vx;
			if (this.players[i].vx<5){
				this.players[i].vx = this.players[i].vx+0.04;
				if (this.players[i].vx>5){this.players[i].vx = 5;}
				}
			else if (this.players[i].vx>5){
				this.players[i].vx = this.players[i].vx-0.04;
				}
			console.log(this.players[i].vx)
			this.players[i].x = Math.floor(this.players[i].xxx/64)+1;
